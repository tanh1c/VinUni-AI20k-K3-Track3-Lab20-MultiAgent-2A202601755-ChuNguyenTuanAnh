...
- Retry transient provider failures with Tenacity.
...
2. LangSmith tracing is enabled when `LANGSMITH_API_KEY` is present. The implementation uses named spans around workflow/agent/provider operations where practical.
...
`.github/workflows/ci.yml` is changed to `workflow_dispatch` only.

Manual input selects:

- `offline`: install dependencies, Ruff lint/format check, mypy, pytest.
- `live`: run offline checks first, then a bounded real-provider smoke test/benchmark using repository secrets.

There are no `push` or `pull_request` triggers.
