...
def test_critic_fact_checks_against_raw_source_evidence() -> None:
    class CapturingLLM:
        ...
        def complete(self, system_prompt: str, user_prompt: str) -> LLMResponse:
            ...
            return LLMResponse(content="Fact-check review completed.")

    state = state_with_answer("Supported claim [S1].")
    llm = CapturingLLM()

    CriticAgent(llm=llm).run(state)

    assert "Sources:" in llm.user_prompt
    assert "[S1] Source" in llm.user_prompt
    assert "Evidence" in llm.user_prompt
